...
import dev.wolveringer.dataclient.protocoll.packets.PacketHandschakeOutStart;
import dev.wolveringer.dataclient.protocoll.packets.PacketInBanStatsRequest;
import dev.wolveringer.dataclient.protocoll.packets.PacketInChangePlayerSettings;
import dev.wolveringer.dataclient.protocoll.packets.PacketInConnectionStatus;
import dev.wolveringer.dataclient.protocoll.packets.PacketInDisconnect;
import dev.wolveringer.dataclient.protocoll.packets.PacketInPlayerSettingsRequest;
import dev.wolveringer.dataclient.protocoll.packets.PacketInServerSwitch;
import dev.wolveringer.dataclient.protocoll.packets.PacketInStatsEdit;
import dev.wolveringer.dataclient.protocoll.packets.PacketInStatsRequest;
import dev.wolveringer.dataclient.protocoll.packets.PacketOutHandschakeAccept;
import dev.wolveringer.dataclient.protocoll.packets.PacketOutPacketStatus;
import dev.wolveringer.dataclient.protocoll.packets.PacketOutPlayerSettings;
import dev.wolveringer.dataclient.protocoll.packets.PacketChatMessage.Target;
import dev.wolveringer.dataclient.protocoll.packets.PacketInConnectionStatus.Status;
import dev.wolveringer.dataclient.protocoll.packets.PacketOutPlayerSettings.SettingValue;
...
public class PacketHandlerBoss {
	...
	public void handle(Packet packet) {
		if (!handschakeComplete) {
			if (packet instanceof PacketHandschakeOutStart) {
				if (!((PacketHandschakeOutStart) packet).getPassword().equals(Main.Password)) {
					owner.disconnect("Password incorrect");
					return;
				}
				owner.host = ((PacketHandschakeOutStart) packet).getHost();
				owner.type = ((PacketHandschakeOutStart) packet).getType();
