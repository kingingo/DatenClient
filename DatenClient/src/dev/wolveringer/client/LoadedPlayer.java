...
public class LoadedPlayer {
	...
	protected synchronized void load() {
		...
		try {
			...
			if (name != null)
				...
				isLoading = false;
				throw new NullPointerException("Cant load player without informations. this -> Name: " + name
						+ ", uuid: " + uuid + ", playerId: " + playerId);
			}
	...
	public void setPremiumSync(boolean active) {
		if (!loaded)
			throw new RuntimeException("Player not loaded. Invoke load() at first");
		PacketInChangePlayerSettings packet = new PacketInChangePlayerSettings(playerId, Setting.PREMIUM_LOGIN,
				active + "");
		handle.writePacket(packet).getSync();
	...
	public ProgressFuture<PacketOutPacketStatus.Error[]> setStats(EditStats... changes) {
		Iterable<PacketInStatsEdit.EditStats> statis = Iterables.filter(Arrays.asList(changes),
				new Predicate<PacketInStatsEdit.EditStats>() {
					@Override
					public boolean apply(PacketInStatsEdit.EditStats arg0) {
						if (arg0 == null)
							return false;
						if (arg0.getAction() == null || arg0.getGame() == null || arg0.getKey() == null
								|| arg0.getValue() == null)
							return false;
						return true;
					}
		...
		return handle
				.writePacket(new PacketInStatsEdit(playerId, FluentIterable.from(statis).toArray(EditStats.class)));
	}
	...
	public ProgressFuture<PacketOutPacketStatus.Error[]> banPlayer(String curruntIp, String banner, String bannerIp,
			UUID bannerUUID, int level, long end, String reson) {
		return handle.writePacket(new PacketInBanPlayer(name, curruntIp, getUUID() + "", banner, bannerIp,
				bannerUUID + "", end, level, reson));
	}
